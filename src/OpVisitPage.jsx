...
export function OpVisitPage() {
  ...
  const startSarvam = async () => {
    ...
    try {
      ...
      const tick = setInterval(() => { const secs = Math.floor((Date.now() - startedAt.current) / 1000); setElapsed(secs); if (secs >= SARVAM_MAX_SECONDS - 5 && secs < SARVAM_MAX_SECONDS) setStatus({ label: `Recording… ${SARVAM_MAX_SECONDS - secs}s remaining`, kind: 'recording' }); if (secs >= SARVAM_MAX_SECONDS) recorder.stop(); }, 1000); recorder._tick = tick;
      recorder.ondataavailable = (event) => { if (event.data.size) chunks.current.push(event.data); };
      recorder.onstop = async () => { clearInterval(tick); stream.getTracks().forEach((track) => track.stop()); mediaStream.current = null; const hitLimit = Math.floor((Date.now() - startedAt.current) / 1000) >= SARVAM_MAX_SECONDS; if (!chunks.current.length) { setStatus({ label: 'No audio recorded.', kind: 'error' }); return; } setIsProcessing(true); setStatus({ label: hitLimit ? '30-second limit reached. Transcribing…' : 'Transcribing with Sarvam…', kind: 'processing' }); const audio = new Blob(chunks.current, { type: 'audio/webm' }); try { const response = await fetch('/api/op-visit/process', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ audioBase64: await blobToBase64(audio), mimeType: 'audio/webm', filename: 'op-visit.webm', languageCode: 'unknown' }) }); const data = await readApiResponse(response); setTranscript(data.transcript); setProposal(data.visit); setSelected({}); setStatus({ label: 'Ready for review', kind: 'success' }); } catch (error) { setStatus({ label: error.message, kind: 'error' }); } finally { setIsProcessing(false); } };
      recorder.start(); setStatus({ label: `Recording… ${SARVAM_MAX_SECONDS}s limit (Sarvam AI)`, kind: 'recording' });
  ...
  const sarvamRemaining = recording && engine === 'sarvam' ? SARVAM_MAX_SECONDS - elapsed : SARVAM_MAX_SECONDS;
  const sarvamWarning = recording && engine === 'sarvam' && sarvamRemaining <= 5;
